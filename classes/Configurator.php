...
class Configurator
{
    //This variable holds the current working directory for config files. It can be modified from constants file.
    ...
    * Sends it to file loading class and then checks if the files are valid or invalid.
    * @param string $ext
    * @return array
    * @throws Throwable
    */
        ...
        try {
            ...
            */
            if (count($filesfromdir) === 0) {
                ...
                * Getting the files above and then filtering them for specific extension is written keeping in mind 
    ...
    /**
    * This method gets the list of valid files to be parsed from session nd the order in which they need to be parsed.
    ...
    */
    public function validate_and_process(array $order)
    {
        try {
            ...
            */
            $files = array_filter(array_combine($order, array_column($_SESSION['validfiles'], 'filename')), 
                        function ($k) {
                            return $k !== 0;
                        }
            ...
            if ($globalconfig = $_SESSION['globalconfig']) {
                ...
                */
                foreach ($keyarr as $key) {
                    ...
                    ? $value->$key //Sends value at next level until the end. Last iteration gives you the final value
