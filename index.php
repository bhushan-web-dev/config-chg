    ...
    if (count($fileswithtype) > 0) {
        ...
        */
        ...
        foreach ($fileswithtype as $file) { 
            ...
            */
            ...
            <br>
            <div><i>Note: <strong>Higher merge order files would overwrite configs from lower ones. 
                Selecting 0 in Merge Order means you don't want to include the file in processing.</strong></i></div>
            <br>
        ...
        */
        if (is_object($configurator->validate_and_process($_POST['processorder']))) {
            ...
            echo 'Sorry, looks like no files were processed or there was an error. Please try again by refreshing.';
        }
    } else if ($_POST && $_POST['submit'] && $_POST['notation']) {
        /**
        * User performed the parsing in previous stage so they can now check for specific config values by providing 
        * dot notation.
        */
        ?>
        <h3>You can now read config values using dotted notation.</h3>
